...
with st.container(border=True):
    ...
    if track == inp.TRACK_MANUAL:
        st.markdown("**Помесячное распределение публикаций верифицированного "
                    "корпуса**")
        st.caption(
            "По каждому месяцу укажите общее число публикаций и число "
            "негативных; позитивные и нейтральные материалы учитываются "
            "суммарно."
        )
        for half in (0, 1):
            cols = st.columns(6)
        ...
        with c2:
            st.number_input(
                ...
                help="Среднее арифметическое пиковых значений трех крупнейших "
                     ...
                     "системы мониторинга.")
        st.markdown("**Помесячные значения медиапоказателя** — для "
...
with st.container(border=True):
    ...
    st.divider()
    st.markdown("**Транспарентность (R_transp)** — чек-лист из десяти "
                "индикаторов")
    ...
    with tc2:
        st.markdown("Блок 2. Компенсаторное раскрытие социально значимой "
                    "информации")
    ...
    st.caption(
        ...
        "официальными источниками на уровне самой компании. Второй блок "
        "операционализирует понятие компенсаторной транспарентности: "
        "раскрытие социально значимых последствий деятельности в сферах, где "
        "публичное информирование допустимо."
    )

    st.divider()
    st.markdown("**Институциональная зрелость (R_inst)** — чек-лист из шести "
                "признаков")
    ...
    with ic2:
        st.markdown("Блок 2. Институциональное закрепление коммуникационной "
                    "функции")
        for i, label in enumerate(oc.INST_BLOCK2):
            st.checkbox(label, key=f"i2_{i}")
    st.caption(
        "Оба чек-листа агрегируются в две ступени: среднее значение "
        "индикаторов внутри блока, затем среднее двух блоков по шкале от 0 до "
        "100. Такой порядок снижает риск двойного учета близких по смыслу "
        "признаков."
    )
...
with st.container(border=True):
    ...
    if st.session_state["ext_on"]:
        st.caption(
            ...
            "фиксирует, подтверждает ли управленческая отчетность "
            "экономическую и социальную отдачу, а также качество бюджетного "
            "исполнения."
        )
...
with st.container(border=True):
    ...
    with tcol:
        ...
        st.markdown(
            f"""
            <div class="oseec-hlabel">{subtitle}</div>
...
if res["critical"] or res["notes"]:
    with st.container(border=True):
        st.markdown("**Диагностические статусы и примечания**")
        if res["critical"]:
            items = "".join(
...
# --- Декомпозиция ----------------------------------------------------------
with st.container(border=True):
    st.markdown("**Декомпозиция оценки**")

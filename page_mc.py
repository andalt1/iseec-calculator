...
st.markdown(
    "Наличие в архитектуре ОСЭЭК теоретически заданных весов, пороговых "
    "значений и поправочных коэффициентов предполагает проверку итогового "
    "результата на устойчивость. Модуль выполняет 10 000 итераций "
    "имитационного моделирования Монте-Карло с фиксированным начальным "
    "значением генератора случайных чисел, что обеспечивает полную "
    "воспроизводимость результатов при повторном запуске."
)
  ...
  with st.container(border=True):
    if abs(meta["base_val"] - res["oseec_b"]) > 1e-9:
        st.markdown(
            ...
            unsafe_allow_html=True)
    st.subheader("Итоги имитационного прогона")
